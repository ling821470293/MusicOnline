<template>
  <div class="sort">
    <div class="bd">
      <form name="listform">
        <a title="点击量" class="curr" rel="nofollow" @click="sortType('-click_num')"><span :class="{search_DESC: cur==='-click_num'}">点击量</span></a>
        <a title="收藏数" class="curr" rel="nofollow" @click="sortType('-fav_num')"><span :class="{search_DESC: cur==='-fav_num'}">收藏数</span></a>
      </form>
    </div>
    <div class="search_num">共<b>{{songNum}}</b>首歌 <span id="search_btn" class="search_btn"></span> </div>
  </div>
</template>
<script>
  export default {
